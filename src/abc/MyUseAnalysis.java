 ...
 */
public class MyUseAnalysis implements LiveLocals
{
     ...
     */
    public MyUseAnalysis(UnitGraph graph)
    {
        if(Options.v().time())
            Timers.v().liveTimer.start();
            
        SimpleUsageAnalysis analysis = new SimpleUsageAnalysis(graph);
...
class SimpleUsageAnalysis extends BackwardFlowAnalysis
{
    FlowSet emptySet;
    Map<Unit, FlowSet> unitToGenerateSet;
    Map<Unit, FlowSet> unitToKillSet;

    SimpleUsageAnalysis(UnitGraph g)
    {
        super(g);
