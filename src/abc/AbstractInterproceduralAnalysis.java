...
public abstract class AbstractInterproceduralAnalysis {
   ...
   /**
	...
	while (it.hasNext()) {
	    ...
	    List<Value> formal_par = getFormalParamaters(m);	    
	    ...
	    /*newly added code begins*/
	    List<Value> actual_args = getActualParamaters(callStmt);
	    FlowSet elem_flowset = (FlowSet) elem;
	    FlowSet elem_clone = elem_flowset.clone();//new ArrayList<String>();
	  //  elem_clone.addAll(elem_list);
	    
	    boolean changed = false;
	    
	    Iterator it1 = elem_flowset.iterator();
	     while(it1.hasNext()){
	    	 Value v = (Value) it1.next();
	    	 if(!formal_par.contains(v)){
	    		 elem_clone.remove(v);
	    		 changed = true;
	    	 }
	     }
	    ...
	    for(int i = 0; i< formal_par.size(); i++){
	    	/*String par = formal_par.get(i).toString();
	    	if(elem_list.contains(par)){
	    		elem_clone.remove(par);
	    		elem_clone.add(actual_args.get(i).toString());
	    	}*/
	    	
	    	Value val = formal_par.get(i);
	    	if(elem_flowset.contains(val)){
	    		elem_clone.remove(val);
	    		elem_clone.add(actual_args.get(i));
	    		changed = true;
	    	}
	    ...
	    if(changed){
	    	applySummary(src, callStmt, elem_clone, accum);
	    	
	    }
	    else
	  //  copy(elem_clone, elem); //can't copy becoz copying changes escape variables for a method in data map.
    ...
    /**
     * newly added method
     * Returns a list of formal parameters of method m 
     * @param m
     * @return
     */
    List<Value> getFormalParamaters(SootMethod m){
    	List<Value> formal_par = new ArrayList<Value>();
    	Body b = m.getActiveBody();
	    
	  //  G.v().out.println("Method : "+b.getMethod().toString());//+ "\n"+"\n" + b.getParameterLocals()+"\n\n");
		
	//	G.v().out.println("no.of par: "+ b.getMethod().getParameterCount());
		for(int i = 0 ; i<b.getMethod().getParameterCount(); i++ ){
			//G.v().out.println((Value)b.getParameterLocal(i));
			formal_par.add((Value)b.getParameterLocal(i));
		}
    ...
    /**
     * newly added method
     * Returns a list of actual arguments passed in function call callStmt
     * @param callStmt
     * @return 
     */
    List<Value> getActualParamaters(Stmt callStmt){
    //	Map<Value, Class> actual_args_map = new HashMap<Value, Class>();
		List<Value> actual_args = new ArrayList<Value>();
